...
export function CreateAdModal(){
	...
	async function handleCreatAd(event: FormEvent) {
		...
		try {
			...
			name: data.name,
			yearsPlayint: Number(data.yearsPlaying),
			weekDays: weekDays.map(Number),
			discord: data.discord,
			houStart: data.hourStart,
					...
					<form onSubmit={handleCreatAd} className='mt-8 flex flex-col gap-4'>
						...
						<div className='grid grid-cols-2 gap-6'>
							<div className='flex flex-col gap-2'>
								...
								<Input name="yearsPlaying" id='yearPlaying' type="number" placeholder='Tudo bem ser ZERO'/>
							</div>
							<div className='flex flex-col gap-2'>
							...
							<div className='flex flex-col gap-2'>
								<label htmlFor="weekDays">Quando costuma jogar?</label>
									...
									>
										...
										className={`w-8 h-8 rounded ${weekDays.includes('2') ? 'bg-violet-500': 'bg-zinc-900'}`} title='Terça'>T</ToggleGroup.Item>
										...
										className={`w-8 h-8 rounded ${weekDays.includes('6') ? 'bg-violet-500': 'bg-zinc-900'}`} title='Sábado'>S</ToggleGroup.Item>
									</ToggleGroup.Root>
							</div>
